...
import numpy as np
#%%
## Simulating the escape rate problem for a fixed variance for LA process
...
def escape (tau, var, N_sample, T):
    ...
    for i in range(0, N_sample):
        y0 = np.random.laplace(0, np.sqrt(var / 2)) #start point of noise is laplace dist.
        x = x0
        y = y0  #starting point is laplace dist. so the process is stationary
        for j in range(0, N):
            if y >= 0:   ## the SDE of Laplace process
                y = y - ((5 * np.sqrt(var)) / (2 * np.sqrt(2) * tau)) * dt + (np.sqrt(5 * var * tau * dt / 2)/ tau) * np.random.normal(0,1)
            elif y < 0: 
                y = y + ((5 * np.sqrt(var)) / (2 * np.sqrt(2) * tau)) * dt + (np.sqrt(5 * var * tau * dt / 2)/ tau) * np.random.normal(0,1)
...
## The simulation must be repeated for different values of D, var
## The results to be found in Fig. 15 in the thesis 
counter =  escape(tau, var , N_sample, T)
r = np.mean(counter) / (T)
